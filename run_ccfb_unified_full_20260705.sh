...
# Usage: run_ccfb_unified_full_20260705.sh <stamp> [<scenes...>] [<sp-mode>]
set -euo pipefail
...
echo "[$(date -Is)] UNIFIED_FULL_START scenes='$SCENES' sp_mode=$SP_MODE" | tee -a "$ROOT/logs/driver.log"
set +e
PYTHONPATH=src $PY examples/run_conceptgraphs_engineered_parity.py \
  --scenes $SCENES \
  --root "$ROOT" \
  --pred-exp-name "$PRED" \
  $CORE $UNIFIED > "$ROOT/logs/run.log" 2>&1
STATUS=$?
set -e
