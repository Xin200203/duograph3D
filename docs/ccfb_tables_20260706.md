...
| **all（8 景聚合，pilot）** | — | **15.32** | **34.86** | — |

**T6 升级为头条主表（n=50 val_50，2026-07-06 晚）**：

| 方法 | ScanNet-50 all mIoU |
| --- | ---: |
| ConceptGraphs（自跑基线，官方参数） | 12.02 |
| **DuoGraph3D（unified，零 ScanNet 调参）** | **26.42（+14.40）** |
| DuoGraph3D w/o gate（消融，进行中） | 待填 |

**⚠️ 归因更正（2026-07-06 晚，见 `ccfb_scannet_attribution_pivot_20260706.md`）**：
substrate 分解证明 +14.5 **不来自机制、也不来自两层记忆巩固**
（wo_gate 26.73 ≈ full 26.42；geomonly 26.54 ≥ memdense 25.98；sp 166 违规仅 2 改标），
而是"我们的批量导出后处理 vs CG 在线合并图"的差异，且混入 20–40× 点密度差。
**ScanNet 降级为 OOD 安全性/鲁棒性检查（机制在真实扫描上不伤害、正确弃权），
不作为精度头条。** 论文头条回到 Replica 的机制因果（T1/T5）。
