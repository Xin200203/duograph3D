...
## 优化战役记分板（2026-07-06 深夜滚动更新）

| 线 | 裁决 | 关键数字 |
| --- | --- | --- |
| O2c 点预算 4× | ❌ 从犯 | office4 −27.2→−22.5（仅回收 5）；room1 +6.29→+2.08（受伤）；room2 无变化 |
| O1 multires-geo 全局 | ⚠️ 备选基底 | office4 治愈（+0.07）但池化 cloth −0.59 等机制可修类拖到 all −4.44 |
| O3 机制上 multires-geo | ❌ 不迁移 | office1 −1.78（比无机制略差）；cloth 修复依赖 dense 桶结构 |
| **O2b 空间连贯性分裂** | ✅ **主犯直击** | **office4 −27.2 → −6.95（+20.3）**；44 巨桶→1130 组件；room2 出分中 |

O2b 机制表述（论文语言）：*spatially incoherent evidence must not be exported
as one semantic entity* ——纯几何判据（key 质心单链接连通分量，eps=0.35m 全局
常数）、每组件从自身 key 重算特征与标签、零场景旋钮。这是 authority 家族的
第四个成员（M4），且其存在性由 D2 尸检（obj#43 墙带巨桶）直接导出。
