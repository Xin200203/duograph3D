...
ROOT = Path("/home/nebula/xxy/duograph3d_artifacts/duograph3d_conceptgraphs_monitored_20260426")
...
PRED_EXP_NAME = "duograph3d_gsa_parity_monitor"
FX = 600.0
FY = 600.0
CX = 599.5
CY = 339.5
...
def world_points_from_mask_arrays(mask: np.ndarray, depth: np.ndarray, rgb_image: np.ndarray, pose: np.ndarray):
    ...
    if len(xs) == 0:
        return None, None, None, {"valid_depth_pixels": 0, "valid_depth_ratio": 0.0}
...
def write_conceptgraphs_payload(scene: str, key_data: dict[str, dict[str, object]], track_assignments: dict[str, list[str]], branch_summary: dict):
    ...
    point_counts = [int(item["point_count"]) for item in export_debug]
    ...
    proxy = {
        ...
        "source": "duograph3d_over_conceptgraphs_gsa_detections_none_monitored",
        "temporal_variant": TemporalVariant.NAIVE_FRAMEWISE.value,
        "readiness": {
...
def write_markdown_report(summary: dict[str, object], path: Path) -> None:
    ...
    for item in scene_debug:
        ...
        lines.append(
            f"| {scene} | {prep['key_count']} | {item['duograph_summary']['memory_node_count']} | "
...
def main() -> None:
    ...
    for scene in args.scenes:
        ...
        manifest, export_debug, missing_keys = write_conceptgraphs_payload(scene, key_data, track_assignments, branch_summary)
    ...
    summary = {
        ...
        "setting_audit": {
            ...
            "monitoring_only_on_top_of_parity_setting": True,
        },
